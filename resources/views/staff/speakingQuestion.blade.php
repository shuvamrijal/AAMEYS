          ...
          <tbody class="tablebody">
            ...
            <tr>
              <td>
                ...
                <div class="images">
                  <?
                  $value=$row->questionfile;
                  if(!empty($value)){
                    $extension=explode('.', $row->questionfile);
                    $file = explode('/public/', $row->questionfile);

                    if($extension[1]=='mp3'){
                      ?>
                      <audio controls controlsList="nodownload">
                        <source src="{{ URL::asset($file[1]) }}" type="audio/mpeg">
                          Your browser does not support the audio element.
                        </audio>
                        <?
                    }else{
                      ?>
                      <img src="{{ URL::asset($file[1]) }}" alt="">
                  <?
                    }
                    ?>
